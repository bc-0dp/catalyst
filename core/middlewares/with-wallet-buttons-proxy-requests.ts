...
export const withWalletButtonsProxyRequests: MiddlewareFactory = (next) => {
  return async (request, event) => {
    ...
    if (request.url.includes('/redirect-to-checkout') && request.method === 'POST') {
      try {
        ...
        const url = redirectData.cart.createCartRedirectUrls.redirectUrls?.redirectedCheckoutUrl;

        if (!url) {
          return NextResponse.json({ error: 'Failed to redirection to checkout pag ' });
        }
